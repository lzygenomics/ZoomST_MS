...
Tables~\ref{tab:representation-summary} and~\ref{tab:hesta-representation-summary} report AMI for eight MOSTA and seven HESTA sections, respectively; Table~\ref{tab:representation-secondary-ari} reports MOSTA ARI from the same three clustering runs. Entries are the mean and sample standard deviation (denominator $3-1$) across three downstream clustering runs with different initializations. For each atlas's summary, sections are averaged equally within each clustering run before computing the mean and standard deviation. These standard deviations quantify clustering variability with fixed representations and sections. Bold marks the top three unrounded values in each comparison, including exact third-place ties. Joint ZoomST features have the highest mean MOSTA ARI (0.345).
...
At each $k\in\{8,32,128\}$, Joint concatenates $e_k$ and $g_k$ and uses the whole-section standardization, PCA32, and three-seed clustering protocol above. HESTA uses its coarse anatomical labels and all 1,129,287 positions across the seven sections. Both HESTA models' exported features pass through float16 and then float32 before feature preprocessing, giving them common stored precision. Table~\ref{tab:cross-granularity-ami}a reports the mean and sample SD of equal-section means across the three downstream clustering runs. Its improvement counts compare the models after averaging these runs within each held-out section. Table~\ref{tab:cross-granularity-ami-sections} gives these paired section means.

\begin{table}[!htbp]
\centering\small
\caption{\textbf{Joint AMI on every held-out section and granularity.} Each value averages downstream clustering runs with seeds 42, 43, and 44; bold marks the higher unrounded value within each section--granularity pair.}
\label{tab:cross-granularity-ami-sections}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.08}
\input{tables/cross_granularity_ami_sections}
\end{table}
...
Table~\ref{tab:cross-granularity-development}b evaluates Joint separately at each context size $k\in\{8,32,128\}$. Within each atlas, DPT uses the same balanced Brain cohort, centered pooled PCA with 32 components and no featurewise standardization, a 30-neighbor graph, and a fixed root for all six model--scale combinations. Each developmental stage is represented by one held-out section. Table~\ref{tab:joint-scale-comparison} additionally retains the MOSTA whole-section anatomical and physical-neighbor readouts alongside DPT.
...
HESTA uses the same centered PCA32, 30-neighbor graph, and ten-component Scanpy DPT definition for both models. The root is the first observation in the sorted sampled CS17 indices, fixed before evaluating the scores; its distances are normalized by the largest finite distance. All Brain observations have finite pseudotime. Carnegie stage numbers specify the ordered target. PCA and graph construction pool the unlabeled Brain observations within each atlas; stage labels anchor the earliest-stage root and evaluate the resulting order.

\paragraph{HESTA cross-method comparison.}
The HESTA panel uses the same 2,560 Brain observations and CS17 root. ZoomST concatenates $e_8,e_{32},e_{128},g_8,g_{32},g_{128}$ and applies centered PCA32. Frozen external representations comprise Novae, SToFM, scGPT-spatial, the Cell, Neighborhood, and Spatial-cell outputs of TERRA, and CellWorld. Each external representation undergoes featurewise z-score standardization over the pooled Brain cohort before PCA32. Every method uses 30 neighbors and the same ten-component Scanpy DPT distance rule. CellWorld uses its frozen CP10K/log1p and per-section Seurat-HVG300 export. Its 512 CS18 observations are disconnected from the common root, so the figure marks its complete-stage readout as NA. The remaining methods cover all five stages. The existing independently fitted per-section DECIPHER embeddings are not pooled into a shared cross-stage coordinate system.

\begin{table}[!htbp]
\centering\scriptsize
\caption{\textbf{MOSTA Joint readouts by granularity.} AMI uses the same downstream clustering mean and SD aggregation as Table~\ref{tab:cross-granularity-ami}a. Neighbor retention is equal-section physical 32-neighbor recovery. Spot-level correlation uses the 4,712-observation Brain cohort. Bold marks the higher value in each pair, including ties.}
