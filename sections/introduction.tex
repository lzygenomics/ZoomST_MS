...
We introduce \method{}, a self-supervised framework that jointly learns granularity-specific tissue representations and transformations between them. Within each granularity, reconstruction and molecular--spatial dual-view alignment ground the representations in molecular content and spatial context. A shared neural ODE transforms molecular states along a continuous log-granularity axis; source and target granularities specify the integration interval. Inspired by learned equivariance~\citep{lenc2015equivariance}, we encourage statistical agreement between the two routes through a \emph{soft distributional equivariance} constraint. We pair each fine representation with either its predicted or its directly encoded coarse counterpart and compare the resulting joint distributions. This constrains fine--coarse associations, not just the distribution of coarse predictions.
