...
\end{equation}
The source bandwidth $\sigma_a^2$ is the median of positive pairwise squared distances among $s_i$, and the destination bandwidth $\sigma_b^2$ is computed from $t_i$. The bandwidth calculations are detached and use a lower bound of $10^{-6}$; if no positive distance is present, the bandwidth is set to one. The numerical kernel denominator is also lower-bounded by $10^{-6}$. Focal-center samples are gathered across devices before computing the statistic. Small negative values from numerical roundoff are clipped to zero, and the three granularity-pair statistics are averaged equally.
